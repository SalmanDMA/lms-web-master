<x-app-layout>
    ...
    @php
        $title = 'Daftar penilaian kelas ' . $class?->name . ' Pelajaran ' . $course->courses_title;
    @endphp
        ...
        <div class="card-body">
            <div class="row mt-3">
                <div class="card col-12 col-md-3 mb-3 mb-md-0">
                    <div
                        class="card-header bg-primary text-white d-flex justify-content-center justify-content-md-start">
                        <span class="fw-bold text-center text-md-start">Total Mengerjakan</span>
                    </div>
                    <div class="card-body border rounded">
                        <h3 class="mb-0 mt-3 fs-1 text-center fw-bolder">
                            {{ $total_user_responses }} / {{ $school_exam->examSetting->maximum_user }}
                        </h3>
                    </div>
                </div>
                <div class="card col-12 col-md-3 mb-3 mb-md-0">
                    ...
                    <table class="table" id="penilaian">
                        ...
                        <tbody>
                            @forelse($student_reports as $report)
                                <tr
                                    data-status="{{ is_null($report['final_grade']) ? 'belum_dinilai' : 'sudah_dinilai' }}">
                                    <td>{{ $report['nisn'] }}</td>
                                    <td>{{ $report['name'] }}</td>
                                    <td>
