<x-app-layout>
 ...
 <div class="card mt-3" id="mainCard">
  ...
  <div class="card-body mt-3">
   <form id="main-form" action="{{ route('staff_curriculum.bank.edit_soal', ['id' => $question_bank->id]) }}" method="POST"
     ...
     <div class="tab-pane fade show active" id="details" role="tabpanel" aria-labelledby="details-tab">
      <div class="mb-3">
       ...
       <textarea class="form-control ckeditor" id="question_text" name="question_text">
                                {{ old('question_text', $question_bank->question_text) }}
                            </textarea>
      ...
      <div class="row mb-3">
       <div class="col-md-6 mb-3 mb-md-0">
        ...
        <select name="course" id="course" class="form-select">
         <option value="" {{ is_null($question_bank->course) ? 'selected' : '' }}>-
          Pilih Pelajaran -</option>
         @foreach ($courses as $course)
          <option value="{{ $course->id }}" {{ $course->id == $question_bank->course ? 'selected' : '' }}>
       ...
       <div class="col-md-6">
        <label for="class_level" class="form-label">Tingkat<span class="text-danger">*</span></label>
        <select name="class_level" id="class_level" class="form-select">
         <option value="" {{ is_null($question_bank->class_level) ? 'selected' : '' }}>
          - Pilih Tingkat
          -</option>
         @foreach ($levels as $level)
          <option value="{{ $level->id }}" {{ $level->id == $question_bank->class_level ? 'selected' : '' }}>
           {{ $level->name }}
          </option>
         @endforeach
        </select>
      ...
      <div class="row mb-3">
       <div class="col-md-6 mb-3 mb-md-0">
        ...
        <div class="input-group">
         <select class="form-select" id="category_id" name="category_id">
          <option value="" {{ is_null($question_bank->category_id) ? 'selected' : '' }}>- Pilih
           Kategori -</option>
          @foreach ($question_category as $item)
           <option value="{{ $item->id }}" {{ $item->id == $question_bank->category_id ? 'selected' : '' }}>
            {{ $item->name }}
           </option>
          @endforeach
         </select>
       ...
       <div class="col-md-6">
        ...
        <select class="form-select" id="question_type" name="question_type" onchange="filterDataChoices()">
         <option value="" {{ is_null($question_bank->question_type) ? 'selected' : '' }}>- Pilih Jenis -
         </option>
         @foreach ($question_type as $item)
          <option value="{{ $item }}" {{ $item == $question_bank->question_type ? 'selected' : '' }}>
           {{ $item }}
          </option>
         @endforeach
        </select>
      ...
      <div class="mb-3 row" id="pointGradeContainer">
       <div class="col-md-12 mb-3">
        ...
        <input class="form-control" type="number" id="point" name="point" min="1"
         value="{{ old('point', $question_bank->point) }}">
        <div class="invalid-feedback">
         Point harus diisi.
        </div>
       </div>
      ...
      <div id="previous-resources-panel" style="display: none;">
       <div id="resources-container-previous">
        ...
        <table class="table table-bordered table-responsive">
         ...
         <tbody>
          @forelse ($question_bank->question_attachment_banks as $index => $resource)
           <tr id="resource-row-{{ $index }}" data-id="{{ $resource->id }}">
            ...
            <td>
             ...
             @elseif ($resource->file_type === 'archive')
              <a href="{{ route('staff_curriculum.bank.download_soal', $resource->id) }}" class="btn btn-success">Unduh</a>
             @else
              @php
    ...
    <input type="hidden" name="shared_at" id="shared_at" value="{{ $question_bank->shared_at }}">
    <input type="hidden" name="shared_count" id="shared_count" value="{{ $question_bank->shared_count }}">
   </form>
  </div>
 </div>
    ...
    <div class="modal-body">
     ...
     <div class="tab-content" id="categoryTabContent">
      ...
      <div class="tab-pane fade" id="add" role="tabpanel" aria-labelledby="add-tab">
       <form id="addCategoryForm" class="mt-3" method="POST" action="{{ route('staff_curriculum.add_soal_kategori') }}">
        @csrf
        <div class="mb-3">
 ...
 <script>
  $(document).ready(function() {
   const questionBank = @json($question_bank);
   let deletedResources = [];
   let existingResources = @json(array_column($question_bank->question_attachment_banks, 'id'));
   let resourceIndex = 1;
   let choiceIndex = 0;
   let inputType = 'radio';
...
`;
   ...
   window.generateChoices = () => {
    const $container = $('#choicesContainer');
    const questionType = $('#question_type').val();
    const gradeMethod = questionBank.grade_method || $('#grade_method').val();
    ...
    // If there are existing choices in questionBank, use them
    const choices = questionBank.choices_banks || [];
   ...
   window.filterDataChoices = () => {
    ...
    if (['Pilihan Ganda', 'Pilihan Ganda Complex', 'True False', 'Essay'].includes(questionType)) {
     ...
     const pointValue = questionBank.point || '';
     const gradeMethodValue = questionBank.grade_method || '';
