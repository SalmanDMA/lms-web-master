<x-app-layout>
    ...
    <x-datatable title="Kelas Ajar">
        <div class="row mb-4 align-items-center">
            <div class="col-12 col-sm-4 col-md-3 d-flex justify-content-start">
                <div class="mb-3">
                    <button type="button" data-bs-toggle="modal" data-bs-target="#addKelasAjarModal"
                    ...
                    <tr data-course-id="{{ $item->course_id }}" data-level-name="{{ $item->class_id }}">
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                <a href="{{ route('staff_curriculum.v_kelas_ajar_student', ['course_id' => $item->course_id, 'sub_class_id' => str_replace('/', '-', $item->sub_class_id)]) }}"
                                    class="btn btn-primary btn-sm">
                                    <i class="bi bi-people-fill"></i> Siswa
                                </a>
                                <form
                                    action="{{ route('staff_curriculum.delete_kelasAjar', ['enrollment_id' => $item->enrollment_id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal fade" id="addKelasAjarModal" tabindex="-1" aria-labelledby="addKelasAjarModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form id="addKelasAjarForm" action="{{ route('staff_curriculum.add_kelas_ajar') }}" method="POST">
                    <div class="modal-body">
                        @csrf
                        <div class="mb-3">
                            <label for="teacher" class="form-label">Guru <span class="text-danger">*</span></label>
                            <select name="teacher_id" class="form-select" id="modalTeacher" onChange="filterSubClass()">
                                <option value="" selected>- Pilih Guru -</option>
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->is_teacher->id }}">{{ $teacher->fullname }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="subClass" class="form-label">Sub Kelas <span
                                    class="text-danger">*</span></label>
    ...
    <script>
        ...
        function filterSubClass() {
            ...
            // Bersihkan opsi subkelas sebelumnya
            subClassSelect.style.display = 'block';
            subClassSelect.innerHTML = '<option value="" selected>- Pilih Sub Kelas -</option>';

            // Jika data guru, course, atau level kosong, maka tidak perlu memproses lebih lanjut
            if (!teacherId || !courseId || !levelId) {
                noSubClassMessage.style.display = 'none';
                return;
            }

            // Ambil daftar subkelas yang sudah diajar oleh guru pada course dan class yang dipilih
            const existingEnrollments = enrollmentData.filter(item =>
                item.teacher_id === teacherId &&
                item.course_id == courseId &&
                item.class_id == levelId
            ).map(item => item.sub_class_id);

            // Filter hanya subkelas yang belum diajar oleh guru
            const filteredSubClasses = subClasses.filter(subClass =>
                subClass.class.id == levelId &&
                !existingEnrollments.includes(subClass.id)
            );
            ...
            // Tambahkan opsi ke dalam select subClass
