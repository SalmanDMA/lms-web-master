...
<x-app-layout>
  ...
  <div class="card">
    <div class="card-header card-header-custom">
      <div class="d-flex align-items-center justify-content-between">
        <span class="mb-0 fs-2 fw-bold font-custom">Edit Tugas</span>
        <span id="title-panel" class="mb-0 fs-4 fw-bold font-custom">( Umum )</span>
      </div>
    </div>
    <div class="card-body card-body-custom">
      <form id="main-form" action="{{ route('teacher.bank.edit_tugas', $assignment->id) }}" method="POST"
        ...
        <div class="tab-content mt-3" id="myTabContent">
          ...
          <div class="tab-pane fade show active" id="umum" role="tabpanel" aria-labelledby="umum-tab">
            <div class="mb-3">
              ...
              <input type="text" name="assignment_title" id="assignment_title" class="form-control form-input-custom"
            ...
            <div class="mb-3">
              <label for="assignment_description" class="form-label font-custom">Deskripsi<span
                  class="text-danger">*</span></label>
              <textarea name="assignment_description" id="assignment_description" class="form-control" rows="3">{{ old('assignment_description', $assignment->assignment_description) }}</textarea>
            </div>

            <div class="mb-3">
              <label for="instruction" class="form-label font-custom">Instruksi<span
                  class="text-danger">*</span></label>
              <textarea name="instruction" id="instruction" class="form-control" rows="3">{{ old('instruction', $assignment->instruction) }}</textarea>
            </div>
                ...
                <select name="courses_name" id="courses_name" class="form-select form-select-custom">
                  ...
                  @foreach ($courses as $course)
                    <option value="{{ $course->id }}"
                      {{ $assignment->courses_name == $course->id ? 'selected' : '' }}>
                ...
                <select name="class_level" id="class_level" class="form-select form-select-custom">
                  ...
                  @foreach ($levels as $level)
                    <option value="{{ $level->id }}"
                      {{ $assignment->class_level == $level->id ? 'selected' : '' }}>
                      {{ $level->name }}</option>
                  @endforeach
                </select>
              </div>
          ...
          <div class="tab-pane fade" id="pengaturan" role="tabpanel" aria-labelledby="pengaturan-tab">
            <div class="mb-3">
              ...
              <input type="number" name="limit_submit" id="limit_submit" class="form-control form-input-custom"
                value="{{ old('limit_submit', $assignment->limit_submit) }}">
            </div>

            <div class="mb-3">
              <label for="due_date" class="form-label font-custom">Batas Waktu Tugas<span
                  class="text-danger">*</span></label>
            ...
            <div class="mb-3">
              ...
              <input type="number" name="max_attach" id="max_attach" max="10" min="1"
                class="form-control form-input-custom" value="{{ old('max_attach', $assignment->max_attach) }}"
            ...
            <div class="mb-3">
              ...
              <select name="is_visibleGrade" id="is_visibleGrade" class="form-select form-select-custom">
                ...
                <option value="1" {{ $assignment->is_visibleGrade == 1 ? 'selected' : '' }}>Ya
                </option>
                <option value="0" {{ $assignment->is_visibleGrade == 0 ? 'selected' : '' }}>Tidak
                </option>
              </select>
            </div>
          ...
          <div class="tab-pane fade" id="resources" role="tabpanel" aria-labelledby="resources-tab">
            ...
            <div id="previous-resources-panel" style="display: none;">
              <div id="resources-container-previous">
                ...
                <table class="table table-bordered table-responsive">
                  ...
                  <tbody>
                    @forelse ($assignment->assignment_attachments as $index => $resource)
                      <tr id="resource-row-{{ $index }}" data-id="{{ $resource->id }}">
                        <td class="font-custom">{{ ucfirst($resource->file_type) }}</td>
                        ...
                        <td>
                          ...
                          @elseif ($resource->file_type === 'archive')
                            <a href="{{ route('teacher.bank.download_tugas', $resource->id) }}"
                    ...
                    @empty
                      <tr>
                        <td colspan="3" class="font-custom text-center">Tidak ada lampiran</td>
                      </tr>
                    @endforelse
                  </tbody>
                </table>
            ...
            <button type="button" class="btn btn-primary-custom" onclick="addResource()" id="btn-add-resource"> <i
                class="bi bi-plus-lg"></i> Tambah Lampiran</button>
            <button type="button" class="btn btn-primary-custom" onclick="toggleResources()"
              id="btn-toggle-resources"> Lihat
              Lampiran
              Sebelumnya</button>
  ...
  <script>
    $(document).ready(function() {
      ...
      let existingResources = @json(array_column($assignment->assignment_attachments, 'id'));
