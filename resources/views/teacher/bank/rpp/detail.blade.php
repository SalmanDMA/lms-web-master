  ...
  <div class="card-body mt-3">
   <ul class="nav nav-tabs" id="rppTab" role="tablist">
    <li class="nav-item" role="presentation">
     <button class="nav-link active" id="umum-tab" data-bs-toggle="tab" data-bs-target="#umum-content" type="button"
      role="tab" aria-controls="umum-content" aria-selected="true">Umum</button>
    </li>
    <li class="nav-item" role="presentation">
     <button class="nav-link" id="daftar-naskah-tab" data-bs-toggle="tab" data-bs-target="#daftar-naskah-content"
      type="button" role="tab" aria-controls="daftar-naskah-content" aria-selected="false">Daftar Naskah
      (Draft)</button>
    </li>
    <li class="nav-item" role="presentation" style="display: none" id="detail-naskah-tab-wrapper">
     <button class="nav-link" id="detail-naskah-tab" data-bs-toggle="tab" data-bs-target="#detail-naskah-content"
      type="button" role="tab" aria-controls="detail-naskah-content" aria-selected="false">Detail Naskah</button>
    </li>
   </ul>
   <div class="row">
    <div class="col-12 border d-flex flex-column justify-content-center align-items-center pt-3">
     ...
     @if ($me->image_path && Storage::exists($linkUrl))
      <img src="{{ Storage::url($linkUrl) }}" alt="Avatar" class="rounded-circle" style="width: 100px; height: 100px">
     @else
    ...
    <div class="tab-content pt-3 mt-3 col-12 border" id="rppTabContent">
     <div class="tab-pane fade show active" id="umum-content" role="tabpanel" aria-labelledby="umum-tab">
      <div class="mb-3">
      ...
      <div class="mb-3">
       ...
       <p><span class="badge" data-semester="{{ $rpp->semester }}">{{ ucfirst($rpp->semester) }}</span></p>
      </div>
      <div class="mb-3">
     ...
     <div class="tab-pane fade" id="daftar-naskah-content" role="tabpanel" aria-labelledby="daftar-naskah-tab">
      <x-datatable title="Daftar Naskah ( Draft )">
       <div class="d-flex justify-content-end mb-4">
        <a href="{{ route('teacher.pengajar.v_add_draft_rpp', ['rpp_id' => $rpp->id]) }}" class="btn btn-primary"
         type="button">Tambah Draft</a>
       </div>
          ...
          <tr>
           ...
           <td>
            <div class="d-flex justify-content-center gap-2">
             <button class="btn btn-primary btn-sm btn-view-draft" data-item-id="{{ $item->id }}">
              <i class="bi bi-eye"></i> Lihat
             </button>
             @if ($latestStatus === 'Dibatalkan' || $latestStatus === 'Ditolak')
              <a
               href="{{ route('teacher.pengajar.v_edit_draft_rpp', ['rpp_id' => $rpp->id, 'rpp_draft_id' => $item->id]) }}"
               class="btn btn-warning btn-sm text-white">
               <i class="bi bi-pencil-square"></i> Ubah
              </a>
              <button class="btn btn-info btn-sm text-white btn-ajukan-ulang" data-bs-toggle="modal"
           ...
           <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
             <div class="modal-header">
              <h5 class="modal-title" id="modalAjukanUlangLabel{{ $item->id }}">Konfirmasi
               Ajukan Ulang</h5>
             ...
             <div class="modal-footer">
              ...
              <form id="ajukan-ulang-form-delete" method="POST"
               ...
               <button type="submit" class="btn btn-primary">Ajukan
                Ulang</button>
               <input type="hidden" name="draft" value="{{ json_encode($item) }}">
              </form>
             </div>
            </div>
           </div>
     ...
     <div class="tab-pane fade" id="detail-naskah-content" role="tabpanel" aria-labelledby="detail-naskah-tab">
    ...
    detailNaskahWrapper.html(`
                    <div id="draft-details">
                        <div class="mb-3 d-flex justify-content-end">
                            ...
                            ${item.status === 'Dibatalkan' ? `` : `<button class="btn btn-danger ms-2" id="btn-batalkan-draft" data-batalkan-url="${batalkanUrl}">Batalkan Draft</button>`}
                            ${item.status === 'Diterima' ? `<a href="${pdfUrl}" class="btn btn-success ms-2">Unduh Naskah</a>` : ''}
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Nama Draft:</label>
                            <p>${item.draft_name}</p>
