<x-app-layout>
  ...
  @php
    $title = 'Ulasan Pembelajaran ' . $class_level['name'] . ' Pelajaran ' . $courses_name['name'];
        ...
        @foreach ($all_questions as $index => $question)
          <div class="card mb-4" style="box-shadow: 0 0 10px 0 rgba(0, 0, 0, 0.15);">
            ...
            <div class="card-body border rounded pt-3">
              <div
                class="d-flex align-items-center {{ is_array($question->section_id) ? 'justify-content-between' : 'justify-content-start' }} mb-3">
                <h5 class="fw-bold mb-0">{!! $question->question_text !!}</h5>
                @if (is_array($question->section_id))
                  <p class="text-muted mb-0">Bagian: {{ $question->section_id['name'] }}</p>
                @endif
              </div>
                ...
                <div>
                  @foreach ($question->choices as $choice)
                    <div class="form-check position-relative">
                      ...
                      <label
                        class="form-check-label w-100 rounded py-2 px-3 {{ $choice->is_true ? 'bg-success text-white fw-bold opacity-100' : '' }}">
                        {!! $choice->choice_text !!}
                        @if ($choice->is_true)
                          <span
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <form method="POST" class="mb-3"
                          action="{{ route('teacher.sekolah.penilaian_ulasan', ['ujian_id' => $ujian_id, 'student_id' => str_replace('/', '-', $student['student_id']), 'response_id' => $response_id]) }}">
                          @csrf
                          @method('PUT')
