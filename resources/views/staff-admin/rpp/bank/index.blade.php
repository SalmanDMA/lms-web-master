<x-app-layout>
        ...
        <table class="table" id="table-rpp-bank">
            ...
            <tbody id="rppTableBodyNoData">
                <tr id="noCourseSelectedMessage">
                    <td colspan="3" class="text-center">Silakan pilih mata pelajaran</td>
                </tr>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addDraftRppModalLabel">Tambah Draft RPP</h5>
                ...
                <form id="addDraftRppForm" action="/teacher/bank/rpp" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="academic_year" class="form-label">Filter Tahun Ajaran</label>
                        ...
                        <div class="mb-3">
                            <label for="draftName" class="form-label">Nama Draft <span
                                    class="text-danger">*</span></label>
    ...
    <script>
        ...
        const academicYearsOptions = generateAcademicYears().map(year => `<option value="${year}">${year}</option>`).join(
            '');
                ...
                `;
                    ...
                    <div class="modal fade" id="editDraftRppModal${index}" tabindex="-1" aria-labelledby="editDraftRppModal${index}Label" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editDraftRppModal${index}Label">Ubah Draft RPP</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <form action="/teacher/bank/rpp/${item.id}" method="POST">
                                    <div class="modal-body">
                                        ...
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="editCourse${index}" class="form-label">Mata Pelajaran <span class="text-danger">*</span></label>
                                            <select class="form-select" id="editCourse${index}" name="course">
                                                <option value="">- Pilih Pelajaran -</option>
                                                ${courses.map(course => `<option value="${course.id}" ${course.id === item.courses ? 'selected' : ''}>${course.courses_title}</option>`).join('')}
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <label for="editLevel${index}" class="form-label">Tingkat <span class="text-danger">*</span></label>
                                            <select class="form-select" id="editLevel${index}" name="level">
                                                <option value="">- Pilih Tingkatan -</option>
                                                ${levels.map(level => `<option value="${level.id}" ${level.id === item.class_level ? 'selected' : ''}>${level.name}</option>`).join('')}
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <label for="editDraftName${index}" class="form-label">Nama Draft <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control" id="editDraftName${index}" name="draftName" value="${item.draft_name}">
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                ...
                `;
                    ...
                    <div class="modal fade" id="deleteDraftRppModal${index}" tabindex="-1" aria-labelledby="deleteDraftRppModal${index}Label" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="deleteDraftRppModal${index}Label">Konfirmasi Hapus Draft RPP</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                ...
                `;
                    ...
                    <div class="modal fade" id="submitDraftRppModal${index}" tabindex="-1" aria-labelledby="submitDraftRppModal${index}Label" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="submitDraftRppModal${index}Label">Ajukan RPP</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <form action="/teacher/bank/rpp/ajukan" method="POST">
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            <label for="ajukanAcademicYear${index}" class="form-label">Tahun Pelajaran <span class="text-danger">*</span></label>
                                            <select class="form-select" id="ajukanAcademicYear${index}" name="academicYear">
                                                <option value="">- Pilih Tahun Pelajaran -</option>
                                                ${academicYearsOptions}
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <label for="ajukanSemester${index}" class="form-label">Semester <span class="text-danger">*</span></label>
                                            <select class="form-select" id="ajukanSemester${index}" name="semester">
